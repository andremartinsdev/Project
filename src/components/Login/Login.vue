                ...
                <div class="card2 card  px-4 py-5">
                  ...
                  <div class="row">  </div>
                    <div class="row mb-4 mt-4 titulo">
                        <div class=""></div> <img :src="logoBms" class="teste2" >
                        <div class=""></div>
                    </div>
                    <div class="row px-3"> <label class="mb-1">
                            <h6 class="mb-0 text-sm">Usuario</h6>
                        </label> <input class="mb-4" type="text" v-model="login.cpfcnpj" name="email" placeholder="Usuario . . ."> </div>
                    <div class="row px-3"> <label class="mb-1">
                            <h6 class="mb-0 text-sm">Senha</h6>
                        </label> <input type="password" name="password" v-model="login.senha" placeholder="Senha . . ."> </div>
                    <div class="row px-3 mb-4">
                    </div>
                    <b-button type="submit" pill block class="btn btn-blue text-center" @click="logar" >Logar</b-button> 
                </div>
      ...
      this.$swal({
        ...
        logar(){
            if(this.login.cpfcnpj === "" || this.login.senha === ""){
                ...
                LoginService.logar(this.login.cpfcnpj, this.login.senha).then(result => {
                    console.log(result)
                    if(result.status === 201 && result.data.liberado === true){
                        this.$router.push('/Home')
                    }else{
                        this.showAlert("info", result.data.mensagem)
                    }
                })
            }
        }
