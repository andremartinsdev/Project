...
export default {
  ...
  methods: {
    ...
    validaErro() {
      const erro = ValidatorPaciente(this.paciente);
      if (erro) {
        if (erro.NOMEPACIENTE) {
          this.showAlert("error", erro.NOMEPACIENTE);
        } else if (erro.TELEFONE) {
          this.showAlert("error", erro.TELEFONE);
        } else if (erro.EMAIL) {
          this.showAlert("error", erro.EMAIL);
        }
      }
